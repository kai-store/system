...
void MessageHandler::NotifyInitialized(Parcel& aParcel)
{
  ...
  int uid = getuid();
  int gid = getgid();
  aParcel.writeInt32(uid);
  aParcel.writeInt32(gid);
...
void MessageHandler::NotifyMPOSReaderModeEvent(Parcel& aParcel, void* aData)
{
  MPOSReaderModeEvent* pEvent = reinterpret_cast<MPOSReaderModeEvent*>(aData);

  aParcel.writeInt32(pEvent->eventType);
  NFCD_DEBUG("enter eventType=%d", pEvent->eventType);
  SendResponse(aParcel);
}
...
void MessageHandler::ProcessRequest(const uint8_t* aData, size_t aDataLen)
{
  ...
  switch (request) {
    ...
    case NFC_REQUEST_TRANSCEIVE:
      ...
      break;
    case NFC_REQUEST_OPEN_CHANNEL:
      HandleOpenChannelRequest(parcel);
      break;
    case NFC_REQUEST_TRANSMIT:
      HandleTransmitRequest(parcel);
      break;
    case NFC_REQUEST_CLOSE_CHANNEL:
      HandleCloseChannelRequest(parcel);
      break;
    case NFC_REQUEST_RESET_SECUREELEMENT:
      HandleResetSecureElementRequest(parcel);
      break;
    case NFC_REQUEST_GET_ATR:
      HandleGetAtrRequest(parcel);
      break;
    case NFC_REQUEST_LS_EXECUTE_SCRIPT:
      HandleLsExecuteScriptRequest(parcel);
      break;
    case NFC_REQUEST_LS_GET_VERSION:
      HandleLsGetVersionRequest(parcel);
      break;
    case NFC_REQUEST_MPOS_READER_MODE:
      HandleMPOSReaderModeRequest(parcel);
      break;

    case NFC_REQUEST_NFC_SELF_TEST:
      HandleNfcSelfTestRequest(parcel);
      break;
...
void MessageHandler::ProcessResponse(NfcResponseType aResponse, NfcErrorCode aError, void* aData)
{
  ...
  switch (aResponse) {
    ...
    case NFC_RESPONSE_TAG_TRANSCEIVE:
      ...
      break;
    case NFC_RESPONSE_OPEN_CHANNEL:
      HandleOpenChannelResponse(parcel, aData);
      break;
    case NFC_RESPONSE_TRANSMIT:
      HandleTransmitResponse(parcel, aData);
      break;
    case NFC_RESPONSE_CLOSE_CHANNEL:
      HandleCloseChannelResponse(parcel, aData);
      break;
    case NFC_RESPONSE_GET_ATR:
      HandleGetAtrResponse(parcel, aData);
      break;
    case NFC_RESPONSE_RESET_SECUREELEMENT:
      HandleResetSecureElementResponse(parcel, aData);
      break;
    case NFC_RESPONSE_LS_EXECUTE_SCRIPT:
      HandleLsExecuteScriptResponse(parcel, aData);
      break;
    case NFC_RESPONSE_LS_GET_VERSION:
      HandleLsGetVersionResponse(parcel, aData);
      break;
    case NFC_RESPONSE_MPOS_READER_MODE:
      HandleMPOSReaderModeResponse(parcel, aData);
      break;
    case NFC_RESPONSE_NFC_SELF_TEST:
      HandleNfcSelfTestResponse(parcel, aData);
      break;
    default:
...
void MessageHandler::ProcessNotification(NfcNotificationType aNotification, void* aData)
{
  ...
  switch (aNotification) {
    ...
    case NFC_NOTIFICATION_NDEF_RECEIVED:
      ...
      break;
    case NFC_NOTIFICATION_MPOS_READER_MODE_EVENT:
      NotifyMPOSReaderModeEvent(parcel, aData);
      break;
    case NFC_NOTIFICATION_RF_FIELD_ACTIVATED_EVENT:
    case NFC_NOTIFICATION_RF_FIELD_DEACTIVATED_EVENT:
      SendResponse(parcel);
      break;
    case NFC_NOTIFICATION_LISTEN_MODE_ACTIVATED_EVENT:
    case NFC_NOTIFICATION_LISTEN_MODE_DEACTIVATED_EVENT:
    case NFC_NOTIFICATION_UNINITIALIZED:
    case NFC_NOTIFICATION_ENABLE_TIMEOUT:
    case NFC_NOTIFICATION_DISABLE_TIMEOUT:
      SendResponse(parcel);
      break;
    default:
      NFCD_ERROR("Not implement");
...
bool MessageHandler::HandleChangeRFStateRequest(Parcel& aParcel)
{
  ...
  int powerMode = aParcel.readInt32();
  bool value;
  switch (rfState) {
    ...
    case NFC_RF_STATE_LISTEN:
      return mService->HandleEnterLowPowerRequest(powerMode);
    default:
      return false;
  }
}
...
bool MessageHandler::HandleOpenChannelRequest(Parcel& aParcel)
{
  NFCD_DEBUG("HandleOpenChannelRequest");
  mService->HandleOpenChannelRequest();
  return true;
}

bool MessageHandler::HandleTransmitRequest(Parcel& aParcel)
{
  int handleId = aParcel.readInt32();
  int bufLen = aParcel.readInt32();

  NFCD_DEBUG("HandleTransmitRequest: length %d", bufLen);
  const void* buf = aParcel.readInplace(bufLen);
  mService->HandleTransmitRequest(handleId, static_cast<const uint8_t*>(buf), bufLen);
  return true;
}

bool MessageHandler::HandleCloseChannelRequest(Parcel& aParcel)
{
  NFCD_DEBUG("HandleCloseChannelRequest");
  int handleId = aParcel.readInt32();
  mService->HandleCloseChannelRequest(handleId);
  return true;
}

bool MessageHandler::HandleResetSecureElementRequest(Parcel& aParcel)
{
  NFCD_DEBUG("HandleResetSecureElement");
  int handleId = aParcel.readInt32();
  mService->HandleResetSecureElementRequest(handleId);
  return true;
}

bool MessageHandler::HandleGetAtrRequest(Parcel& aParcel)
{
  NFCD_DEBUG("HandleGetAtrRequest");
  int handleId = aParcel.readInt32();
  mService->HandleGetAtrRequest(handleId);
  return true;
}
...
bool MessageHandler::HandleLsExecuteScriptRequest(Parcel& aParcel)
{
  std::string lsScriptFile = StrdupReadString(aParcel);
  std::string lsResponseFile = StrdupReadString(aParcel);
  std::string appID = StrdupReadString(aParcel);

  NFCD_DEBUG("file1 %s file2 %s if %s", lsScriptFile.c_str(), lsResponseFile.c_str(), appID.c_str());
  mService->HandleLsExecuteScriptRequest(lsScriptFile, lsResponseFile, appID);
  return true;
}

bool MessageHandler::HandleLsGetVersionRequest(Parcel& aParcel)
{
  mService->HandleLsGetVersionRequest();
  return true;
}

bool MessageHandler::HandleMPOSReaderModeRequest(Parcel& aParcel)
{
  int mPOSReaderMode = aParcel.readInt32();
  bool enabled = mPOSReaderMode;
  NFCD_DEBUG("HandleMPOSReaderModeRequest: mPOSReaderMode %d enabled %d", mPOSReaderMode, enabled);
  mService->HandleMPOSReaderModeRequest(enabled);
  return true;
}

bool MessageHandler::HandleNfcSelfTestRequest(Parcel& aParcel)
{
  int type = aParcel.readInt32();
  NFCD_DEBUG("HandleNfcSelfTestRequest: type %d", type);
  mService->HandleNfcSelfTestRequest(type);
  return true;
}
...
bool MessageHandler::HandleOpenChannelResponse(Parcel& aParcel, void* aData)
{
  int handle = *reinterpret_cast<int*>(aData);
  aParcel.writeInt32(handle);
  SendResponse(aParcel);
  NFCD_DEBUG("HandleOpenChannelResponse= handle id = %d", handle);
  return true;
}

bool MessageHandler::HandleTransmitResponse(Parcel& aParcel, void* aData)
{
  TransmitResponse* transmitRes = reinterpret_cast<TransmitResponse*>(aData);
  int handle = transmitRes->handle;
  std::vector<uint8_t>* response = reinterpret_cast<std::vector<uint8_t>*>(transmitRes->response);
  ...
  if (response) {
    ...
    aParcel.writeInt32(0);
  }

  SendResponse(aParcel);
  NFCD_DEBUG("HandleTransmitResponse: handle id = %d", handle);
  return true;
}

bool MessageHandler::HandleCloseChannelResponse(Parcel& aParcel, void* aData)
{
  int handle = *reinterpret_cast<int*>(aData);
  NFCD_DEBUG("HandleCloseChannelResponse= handle id = %d", handle);
  aParcel.writeInt32(handle);
  SendResponse(aParcel);
  return true;
}

bool MessageHandler::HandleGetAtrResponse(Parcel& aParcel, void* aData)
{
  TransmitResponse* transmitRes = reinterpret_cast<TransmitResponse*>(aData);
  int handle = transmitRes->handle;
  std::vector<uint8_t>* atr = reinterpret_cast<std::vector<uint8_t>*>(transmitRes->response);

  aParcel.writeInt32(handle);

  if (atr) {
    uint32_t length = atr->size();
    aParcel.writeInt32(length);
    uint8_t* buf = static_cast<uint8_t*>(aParcel.writeInplace(length));
    std::copy(atr->begin(), atr->end(), buf);
  } else {
    aParcel.writeInt32(0);
  }

  SendResponse(aParcel);
  NFCD_DEBUG("HandleGetAtrResponse= handle id = %d", handle);
  return true;
}

bool MessageHandler::HandleResetSecureElementResponse(Parcel& aParcel, void* aData)
{
  int handle = *reinterpret_cast<int*>(aData);
  NFCD_DEBUG("HandleGetAtrResponse= handle id = %d", handle);
  aParcel.writeInt32(handle);
...
bool MessageHandler::HandleLsExecuteScriptResponse(Parcel& aParcel, void* aData)
{
  std::vector<uint8_t>* response = reinterpret_cast<std::vector<uint8_t>*>(aData);
  uint32_t length = response->size();
...
bool MessageHandler::HandleLsGetVersionResponse(Parcel& aParcel, void* aData)
{
  std::vector<uint8_t>* response = reinterpret_cast<std::vector<uint8_t>*>(aData);
  uint32_t length = response->size();
...
bool MessageHandler::HandleMPOSReaderModeResponse(Parcel& aParcel, void* aData)
{
  NFCD_DEBUG("HandleMPOSReaderModeResponse");
  SendResponse(aParcel);
  return true;
}

bool MessageHandler::HandleNfcSelfTestResponse(Parcel& aParcel, void* aData)
{
  NFCD_DEBUG("HandleNfcSelfTestResponse");
  SendResponse(aParcel);
  return true;
}
