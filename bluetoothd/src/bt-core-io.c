...
/* maximum number of pending wake alarm */
#define MAX_PENDING_WAKE_ALARM 30

static int pending_wake_alarm_count;
...
// It's 'OR' here since AOSP Marshmallow supports min_16_digit as well.
#if defined(Q_BLUETOOTH) || ANDROID_VERSION >= 23
static void
pin_request_cb(bt_bdaddr_t* remote_bd_addr, bt_bdname_t* bd_name,
               uint32_t cod, bool min_16_digit ATTRIBS(UNUSED))
#else
static void
pin_request_cb(bt_bdaddr_t* remote_bd_addr, bt_bdname_t* bd_name,
               uint32_t cod)
                  ...
                  void* data)
{
  ...
  if (pending_wake_alarm_count > MAX_PENDING_WAKE_ALARM) {
    ALOGW("Skip a wake alarm since there are many pending alarms.");
    return false;
  }
...
// Assume Marshmallow has guest_mode argument since most of AOSP branches do.
#if ANDROID_VERSION >= 23
  // KaiOS doesnt't support guest mode
  status = bt_interface->enable(false /* restricted mode */);
#else
