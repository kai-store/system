...
#if defined(Q_BLUETOOTH) && ANDROID_VERSION >= 23
static bt_bdaddr_t connected_addr;
#endif
...
static void
#if defined(Q_BLUETOOTH) && ANDROID_VERSION >= 23
get_play_status_cb(bt_bdaddr_t *bd_addr ATTRIBS(UNUSED))
#else
...
static void
#if defined(Q_BLUETOOTH) && ANDROID_VERSION >= 23
list_player_app_attr_cb(bt_bdaddr_t *bd_addr ATTRIBS(UNUSED))
#else
...
static void
#if defined(Q_BLUETOOTH) && ANDROID_VERSION >= 23
get_player_app_values_text_cb(uint8_t attr_id, uint8_t num_val, uint8_t* p_vals,
                              bt_bdaddr_t *bd_addr ATTRIBS(UNUSED))
#else
get_player_app_values_text_cb(uint8_t attr_id, uint8_t num_val, uint8_t* p_vals)
#endif
...
static void
#if defined(Q_BLUETOOTH) && ANDROID_VERSION >= 23
passthrough_cmd_cb(int id, int key_state, bt_bdaddr_t *bd_addr ATTRIBS(UNUSED))
#else
...
#if defined(Q_BLUETOOTH) && ANDROID_VERSION >= 23
static void
connection_state_cb(bool state, bt_bdaddr_t *bd_addr)
{
  if (state == true) {
    bt_rc_set_connected_addr(bd_addr);
  }
}
#endif
...
#if defined(Q_BLUETOOTH) && ANDROID_VERSION >= 23
  status = btrc_interface->get_player_app_value_rsp(&p_vals, &connected_addr);
#else
...
#if defined(Q_BLUETOOTH) && ANDROID_VERSION >= 23
  status = btrc_interface->set_player_app_value_rsp(rsp_status, &connected_addr);
#else
...
#if defined(Q_BLUETOOTH) && ANDROID_VERSION >= 23
  status = btrc_interface->set_volume(volume, &connected_addr);
#else
...
#if defined(Q_BLUETOOTH) && ANDROID_VERSION >= 23
void
bt_rc_set_connected_addr(bt_bdaddr_t* bd_addr)
{
  connected_addr = *bd_addr;
}
#endif
...
#if defined(Q_BLUETOOTH) && ANDROID_VERSION >= 23
  status = btrc_interface->init(&btrc_callbacks, /* max_avrcp_connections */ 1);
#else
