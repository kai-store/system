...
static void
connection_priority_cb(bt_bdaddr_t* bd_addr)
{
  bt_av_allow_connection(1, bd_addr);
}
#endif
...
#if defined(Q_BLUETOOTH) && ANDROID_VERSION >= 23
void
bt_av_allow_connection(int is_valid , bt_bdaddr_t *bd_addr)
{
  assert(btav_interface);
  assert(btav_interface->allow_connection);
  assert(bd_addr);

  btav_interface->allow_connection(is_valid, bd_addr);
}
#endif
                 ...
                 void (*send_pdu_cb)(struct pdu_wbuf*)))(const struct pdu*)
{
...
#endif
#if defined(Q_BLUETOOTH) && ANDROID_VERSION >= 23
    .connection_priority_cb = connection_priority_cb,
#endif
  };
  ...
  status = btav_interface->init(&btav_callbacks,
                                1,  /* max_a2dp_connections */
                                0); /* a2dp_multicast_state */
#else
  status = btav_interface->init(&btav_callbacks);
#endif
