...
#if ANDROID_VERSION >= 23
  if ((append_to_pdu(&wbuf->buf.pdu, "iii",
                     (int32_t)p_track_adv_info->client_if,
                     (int32_t)p_track_adv_info->filt_index,
                     (int32_t)p_track_adv_info->addr_type) < 0) ||
      (append_bt_bdaddr_t(&wbuf->buf.pdu, &(p_track_adv_info->bd_addr)) < 0) ||
      (append_to_pdu(&wbuf->buf.pdu, "i",
                     (int32_t)p_track_adv_info->advertiser_state) < 0))
#else
...
#if ANDROID_VERSION >= 23
  btgatt_filt_param_setup_t param;
  // Don't read num_of_tracking_entries since it's not supported in framework.
  off = read_pdu_at(cmd, 0, "iiiiiiiiiiii", &param.client_if,
                                            &param.action,
                                            &param.filt_index,
                                            &param.feat_seln,
                                            &param.list_logic_type,
                                            &param.filt_logic_type,
                                            &param.rssi_high_thres,
                                            &param.rssi_low_thres,
                                            &param.dely_mode,
                                            &param.found_timeout,
                                            &param.lost_timeout,
                                            &param.found_timeout_cnt);
#else
...
#if ANDROID_VERSION >= 23
  // Set the default value of tracking enties to 0.
  param.num_of_tracking_entries = 0;
  status = btgatt_interface->client->scan_filter_param_setup(param);
#else
...
#if ANDROID_VERSION >= 23
  if (read_pdu_at(cmd, 0, "iii", &client_if, &interval, &window) < 0) {
#else
...
#if ANDROID_VERSION >= 23
  status = btgatt_interface->client->set_scan_parameters(client_if, interval, window);
#else
