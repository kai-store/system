...
/* START [D18022201] - Do tag disconnect for presence check command */
static bool            sIsTagPresent = true;
/* END [D18022201] - Do tag disconnect for presence check command */
...
int NfcTagManager::DoCheckNdef(int aNdefInfo[])
{
  ...
  if (sCheckNdefStatus == NFA_STATUS_OK) {
    ...
    if (sCheckNdefCardReadOnly)
      ...
      aNdefInfo[1] = NDEF_MODE_READ_WRITE;
/* START [D18022001] Tag write for broken NDEF TAG */
    if (sCheckNdefCurrentSize != 0)
/* END [D18022001] Tag write for broken NDEF TAG */
...
{
/* START [D18022201] - Do tag disconnect for presence check command */
  sIsTagPresent = aStatus == NFC_STATUS_OK;
/* END [D18022201] - Do tag disconnect for presence check command */
  if (aStatus == NFA_STATUS_OK) {
    sCountTagAway = 0;
  } else {
...
bool NfcTagManager::DoWrite(std::vector<uint8_t>& aBuf)
{
  ...
  for (size_t i = 0; i < aBuf.size(); i++) { /* [D18022201] */
    p_data[i] = aBuf[i];
  }
