...
// Refer to section statm in http://man7.org/linux/man-pages/man5/proc.5.html.
struct Statm {
  // Total program size.
  unsigned long size;
  // Resident set size.
...
void readMemoryStat(Statm& result)
{
  unsigned long dummy;
  ...
  if(7 != fscanf(f,"%ld %ld %ld %ld %ld %ld %ld", &result.size, &result.resident,
...
int getMemory(void) {
  readMemoryStat(memoryStat);
  return memoryStat.size;
}
